...
def generate_mermaid() -> str:
    ...
    wf = _get_compiled_graph()
    return wf.get_graph().draw_mermaid()  # type: ignore[no-any-return]
...
def generate_ascii_table() -> str:
    ...
    rows = [
        ...
        (
            "generator",
            "LLM",
            "Produces summary + answer + citations (ResearchAgent or SupportAgent)",
        ),
    ...
    for row in rows:
        lines.append("| " + " | ".join(v.ljust(w) for v, w in zip(row, col_w, strict=False)) + " |")
    lines.append(sep)
    return "\n".join(lines)
